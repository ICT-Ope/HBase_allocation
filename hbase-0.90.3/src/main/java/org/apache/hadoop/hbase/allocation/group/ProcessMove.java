...
public class ProcessMove implements Runnable {
  ...
  public ProcessMove(Set<MoveGroupPlan> movegroupPlanset,
      ...
      Map<String, Boolean> grouppropertymap, HMaster master, String currentdir) {
    super();
  ...
  public void run() {
    ...
    for (MoveGroupPlan plan : movegroupPlanset) {
      ...
      if (regionlist != null) {
        LOG.info("We have " + regionlist.size() + " regions to move ...");
        // move region
        while (true) {
          ...
          for (HRegionInfo hri : regionlist) {
            ...
            for (HServerInfo hsr : availalbeserver) {
              ...
              if (!willmoveservers.contains(name)) {
                ...
                LOG.info("Available server " + name + "for table " + tablename);
              }
            }
            LOG.info("Available server  length " + availservernames.size());
            if (availservernames.size() <= 0) {
              LOG.info("No available regionservers left for  table" + tablename
            ...
            Collection<HServerInfo> serverInfo = master.getClusterStatus()
                .getServerInfo();
            ...
            String regionName = Bytes.toStringBinary(hri.getRegionName());
            String encodeRegionName = regionName.substring(
                (regionName.lastIndexOf(".", regionName.length() - 2)) + 1,
                regionName.length() - 1);
            try {
              if (!hri.isOffline() && !hri.isSplit()) {
                synchronized (GroupAssignmentManager.class) {
                  master.move(Bytes.toBytes(encodeRegionName),
                      Bytes.toBytes(targetserver.getServerName()));
                }
                LOG.info("Move region " + encodeRegionName + " from server "
                    + servername + " to server " + targetserver.getServerName());
              }
      ...
      if (grouppropertymap.get(targetgp) || grouppropertymap.get(originalgp)) {
      ...
      try {
        ServerWithGroup.writeGroupInfo(master, GROUP_INFORMATION, groupmap,
            grouppropertymap);
