...
public class ServerWithGroup {
  ...
  /**
   * Used for get current processing regionserver when change regionserver group
   * 
   ...
   * 
   * @return true if system is moving regions between regionservers , else false
   */
  ...
  /**
   * Used for change regionserver group , {@link MoveGroupPlan} will be done by
   * a thread {@link ProcessMove} , it will be used in front-end
   * 
   ...
   * <p>
   * System will delete offline regionserver from configuration file and add new
   * online regionserver into DEFAULT_GROUP
   * 
   * @param master
   *          HMaster interface
   * @param confline
  ...
  /**
   * Get group property grouppropertymap from configuration line ,if this group
   * is special ,it will be set true , and when move regionserver groups ,it
   * will force update new configuration and jars
   * 
   * @param master
   *          HMaster interface
   * @param confline
   ...
   * @param groupmap
   *          Map< String, List< String >> key:groupname , value: regionserver
   *          name list
  ...
  public static void writeGroupInfo(HMaster master, final String confpath,
      Map<String, List<String>> groupmap, Map<String, Boolean> grouppropertymap)
      throws IOException {
    // write back to hdfs
    FileSystem fs = master.getMasterFileSystem().getFileSystem();
    URI hdfsuri = master.getMasterFileSystem().getRootDir().toUri();
   ...
   * @param servername
   *          regionserver name
   * @return regionlist List< HRegionInfo > , List of regions this regionserver
   *         hold
   */
  public static List<HRegionInfo> listRegionsInServerFromMeta(HMaster hmaster,
      String servername) {
    ...
    try {
      ...
      while ((rt = rs.next()) != null) {
        ...
        if (!regionlist.contains(region)) {
          HServerAddress addr = new HServerAddress(Bytes.toString(address));
   ...
   *           zookeeper connection exception
   */
  public static List<HTableDescriptor> listTableOnRegionServer(HMaster master,
      String servername) throws MasterNotRunningException,
      ZooKeeperConnectionException {
    List<HRegionInfo> regionlist = listRegionOnRegionServer(master, servername);
  ...
  public static int getRegionNumOnGroup(HMaster hmaster, String groupname,
    ...
    for (String server : serverlist) {
      try {
        List<HRegionInfo> regions = listRegionOnRegionServer(hmaster, server);
