				...
				if (command.equals("distconf")) {
					...
					for (String serveraddr : groupserverlist) {
						try {
							...
							System.out.println("Before Distribute "+addr+" configuration");
							//distribute new configuration
							moveimpl.DistributeConf(addr,
									"distribute");
					...
					synchronized (GroupAssignmentManager.class) {
						...
						for (String serveraddr : groupserverlist) {
							try {
								String addr = serveraddr.substring(0,serveraddr.indexOf(","));
								System.out.println("Before Restart "+addr+" configuration");
								moveimpl.ImplRegionServer(addr,
										"stop");
								moveimpl.ImplRegionServer(addr,
										"start");
						...
						while(true){
							...
							for (HServerInfo info: servers){
								String onlineserver = info.getHostname()+","+info.getServerAddress().getPort();
					...
					if (submit.equals("Distribute")) {
						if (configuration != null
							...
							Thread DistributeThread = new Thread(
									new Distribute(master,groupserverlist,
											CURRENTDIR, "distconf"));
						...
						Thread RestartThread = new Thread(new Distribute(
								master,groupserverlist, CURRENTDIR, "restart"));
